...
export function render(src, where = 'markdown', ctx = {}) {
  ...
  <img src="${s.src}" width="${s.width}" height="${s.height}" loading="lazy" decoding="async" alt="${s.alt.replace(/"/g, '&quot;')}">
    ...
    /* `@mymap` drops in the marking tool, generated by build.mjs from areas.mjs so the
       twenty rows cannot drift from the twenty areas. Hard error if the context is
       missing, for the same reason @slide hard-errors: a tool that renders as nothing
       is a page that still looks fine. */
    if (line.trim() === '@mymap') {
      if (!ctx.mymap) {
        throw new Error(`md: @mymap in ${where} (line ${i + 1}) but no tool markup was supplied — build.mjs must pass ctx.mymap`);
      }
      out.push(ctx.mymap);
      i++; continue;
    }

    /* `@canvas` drops in the map builder, generated by build.mjs from shapes.mjs and
       areas.mjs. Hard error when the context is missing, like @mymap and @slide: a
       page that silently renders its tool as nothing still looks fine. */
    if (line.trim() === '@canvas') {
      if (!ctx.canvas) {
        throw new Error(`md: @canvas in ${where} (line ${i + 1}) but no tool markup was supplied — build.mjs must pass ctx.canvas`);
      }
      out.push(ctx.canvas);
      i++; continue;
    }
