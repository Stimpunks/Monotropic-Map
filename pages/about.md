# About this site

## Who made the map

**The Map of Monotropic Experiences was created by Helen Edgar** of [Autistic Realms](https://autisticrealms.com) in 2024, as a reflection of her own monotropic bodymind experiences.

It was developed further as a collaboration between Helen Edgar and the **Stimpunks Foundation** — Ryan Boren, Norah Hobbs and Chelsea Adams. Created by and for Autistic people.

This site is a joint Stimpunks and Autistic Realms project, like [Star Stuff](https://starstuff.earth/) and [Queering Earth](https://queering.earth/).

## What monotropism is

Monotropism is a neurodiversity-affirming theory of autism, developed by Autistic people — initially by Dinah Murray and Wenn Lawson, with Mike Lesser.

Monotropic minds pull attention more strongly toward a smaller number of interests at any given time, leaving fewer resources for everything else. Things outside the attention tunnel get missed, and moving between tunnels is expensive.

Autistic, ADHD and AuDHD people are more likely to be monotropic.

> Monotropism is one of the key ideas required for making sense of autism, along with the double empathy problem and neurodiversity.
> — [monotropism.org](https://monotropism.org/)

The canonical home of the theory is **[monotropism.org](https://monotropism.org/)**, maintained by Fergus Murray. This site maps one set of experiences; that site is the theory.

## The map is a map of other people's words

Almost every area on this map was named by somebody, and the naming is the content. Erin Human on tendril theory. Fergus Murray on attention tunnels and flow. Damian Milton on the double empathy problem. Tanya Adkin and David Gray-Hammond on meerkat mode. Dora Raymaker on Autistic burnout. Wenn Lawson on object permanence. Helen Edgar on penguin pebbling, monotropic time, and the Autistic rhizome. Ryan Boren on Cavendish Space.

Every area page credits its originator and links to their own words. Where an area is a shared community coinage with no single author, the page says that rather than leaving a blank.

## The licence

The map is **[CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)**: *Map of Monotropic Experiences* by Helen Edgar.

Everything on this site derived from the map carries the same licence. You can share it and adapt it, including commercially. You must keep the credit, and you must license what you build under CC BY-SA 4.0 too.

**We cannot add a non-commercial restriction to it and neither can anyone else** — ShareAlike does not allow an adaptation to be more restrictive than the original. If you have seen our other work under a non-commercial licence, this one is deliberately different.

## What this site does not do

- **No Monotropism Questionnaire here.** The MQ is a validated self-report measure developed by Garau et al. in 2023, with 47 items written by Autistic adults from lived experience and academic expertise. You can [take it online](https://pa11erns.com/psychometrics/mq/), hosted elsewhere with its authors cited. We link it. Copying somebody else's validated instrument would invite people to read our numbers as theirs.
- **No score, no type, no result.** This is a map, not an assessment.
- **No accounts, no analytics, no tracking.** See [privacy](privacy.html).

## Elsewhere on the map

Some areas of the map have grown their own sites.

- [Penguin Pebbling](https://penguinpebbling.app/) — area 2, the Cove of Friendship. A neuro-affirming card game by Helen Edgar and Ryan Boren.
- [Cavendish Cards](https://cavendish.space/deck.html) — area 10, the Campsite. An open deck for naming sensory and interaction needs.
- [Star Stuff](https://starstuff.earth/) — areas 8 and 17, the Rhizomatic Communities and the Forest of Joy, Awe and Wonder.
